"""
ResNet-style temperature classification model.

Same task as the other example models (cold / normal / hot classification
from a window of 5 temperature readings), but uses residual (skip) connections
with Conv2D + BatchNormalization + element-wise Add to exercise the
non-sequential / DAG architecture support in the ml2plc pipeline.

Key layers exercised:
    - Conv2D
    - BatchNormalization  (inference-mode: per-channel affine)
    - Element-wise Add    (skip / residual connection)
    - GlobalAveragePool
    - Flatten
    - Dense

The architecture is intentionally kept very small so the resulting
Structured Text fits comfortably in PLC memory (~96 KB).

Usage:
    python examples/models/resnet_model.py [model_name]

This will:
    1.  Create & train the ResNet-style model
...
class ResNetTemperatureModel(MLModel):
    ...
    Architecture:
        Input (5, 1)
          ...
          → Conv2D(8, (1,3), same)  → BatchNorm → ReLU        # stem

          ┌─ skip ─────────────────────────────────────────┐
          │  Conv2D(8, (1,3), same)  → BatchNorm → ReLU    │   # residual block
          │  Conv2D(8, (1,3), same)  → BatchNorm            │
          └─ Add ← ────────────────────────────────────────┘
          → ReLU
    ...
    def create_model(self) -> tf.keras.Model:
        """Build a tiny ResNet model that fits in PLC memory."""
        ...
        # ── Stem: project 1 channel → 8 channels ─────────────────────────
        x = tf.keras.layers.Conv2D(
            8, (1, 3), padding="same", use_bias=False, name="stem_conv"
        )(x)
        x = tf.keras.layers.BatchNormalization(name="stem_bn")(x)
        x = tf.keras.layers.ReLU(name="stem_relu")(x)

        # ── Residual block (channels stay 8, so shortcut is identity) ────
        shortcut = x

        x = tf.keras.layers.Conv2D(
            8, (1, 3), padding="same", use_bias=False, name="res_conv1"
        )(x)
        x = tf.keras.layers.BatchNormalization(name="res_bn1")(x)
        x = tf.keras.layers.ReLU(name="res_relu1")(x)

        x = tf.keras.layers.Conv2D(
            8, (1, 3), padding="same", use_bias=False, name="res_conv2"
        )(x)
        x = tf.keras.layers.BatchNormalization(name="res_bn2")(x)

        # Skip connection: element-wise add
        x = tf.keras.layers.Add(name="res_add")([x, shortcut])
        x = tf.keras.layers.ReLU(name="res_relu2")(x)

        # ── Head: global pool → dense classifier ─────────────────────────
        ...
        self.model = tf.keras.Model(
            inputs=inputs, outputs=outputs, name="resnet_temp_model"
        )
        self.model.compile(
    ...
    def train(
        ...
        print(f"\n{'='*50}")
        print(f"Training ResNet model  —  {len(X_train)} train / {len(X_val)} val")
    ...
    def demo(self):
        ...
        print(f"\n{'='*55}")
        print("  ResNet Temperature Sensor Demo")
        print(f"{'='*55}")
...
def compile_to_st(onnx_path: Path):
    ...
    subprocess.run(
        [sys.executable, "src/codegen/main.py", str(onnx_path), "-v"],
        check=True,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Train a ResNet-style temperature classifier and compile to ST."
    )
    parser.add_argument(
        "model_name",
        nargs="?",
    ...
    model = ResNetTemperatureModel(model_name=args.model_name)
