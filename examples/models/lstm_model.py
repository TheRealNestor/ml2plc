"""
LSTM-based temperature classification model.

Task: Classify temperature sequences as cold / normal / hot based on
a window of historical readings using an LSTM (Long Short-Term Memory) network.

The LSTM maintains hidden state across timesteps, making it suitable
for sequences with temporal dependencies.
...
Usage:
    python examples/models/lstm_model.py [--model-name name] [--epochs N]

This will:
    1. Create & train the LSTM model
...
class LSTMTemperatureModel:
    ...
    def create_model(self) -> tf.keras.Model:
        ...
        # LSTM layer: processes sequence and outputs final hidden state
        x = tf.keras.layers.LSTM(32, return_sequences=False, name="lstm_layer")(inputs)
        ...
        self.model = tf.keras.Model(
            inputs=inputs, outputs=outputs, name="lstm_temp_model"
        )
        self.model.compile(
    ...
    def compile_to_st(self, onnx_path: str, output_path: str | None = None) -> str:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # Show any warnings/debug output
                if result.stderr:
                ...
                # Check if compilation had errors embedded in the ST code
                if "Error lowering region" in st_code:
                    print(f"⚠ Warning: Errors detected in generated code")
    ...
    def demo(self):
        ...
        print("\n" + "=" * 60)
        print("DEMO: LSTM Predictions on sample sequences")
        print("=" * 60)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Create and compile LSTM model for temperature classification"
    )
...
def main():
    ...
    print("=" * 60)
    print("LSTM Temperature Classification Model")
    print("=" * 60)

    # Create model
    model = LSTMTemperatureModel(
